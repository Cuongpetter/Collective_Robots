...
class AutoMapper(Node):
    ...
    def scan_callback(self, msg):
        # "Front Cone" of vision: Detect vision in range of 30 deg left (0 -> 30) AND 30 deg right (330 -> 360)
        ...
        # --- LOGIC ---
        if min_front_dist < 0.25:  # Size of burger LxWxH: 138mm x 178mm x 192mm 
            # Turn Left when detect Obstacle infront  
            self.move_cmd.linear.x = 0.0	# m/s
            self.move_cmd.angular.z = 0.5 	# rad/s
            self.get_logger().info(f'Obstacle detected at {min_front_dist:.2f}m! Turning...')
        else:
            # Drive Forward when no Obstacle infront
